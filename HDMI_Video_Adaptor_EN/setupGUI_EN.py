...
if len(dev_TTY) == 0:
    dev_TTY = [""]
    sg.PopupNoButtons("No serial ports found", auto_close=True)
    sys.exit()


# Draw the button
namesMode = ('Self-synchronization', 'External frequency', 'From Z80 pin 6')
...
sliderF = sg.Frame(layout=[[
    sg.Slider((4000, 11000), orientation='h', enable_events=True, key='-SLIDERF-', size=(250, 20))
...
]], title="VS Length", title_location=sg.TITLE_LOCATION_TOP, key='settings_VS_LEN_Block')
...
syncSelect = sg.Frame(layout=[[sg.Radio('Separate', "SYNCMODE", default=True, key="isVS_HS"),
                               sg.Radio('Composite', "SYNCMODE", key="isVHS")],
                              [sg.Push(), slider_VS_LEN, sg.Push()]],
                      title="Synchronization", title_location=sg.TITLE_LOCATION_TOP)
...
block0 = [
    [sg.Frame(layout=[[sg.Radio('VGA', "VIDEOMODE", default=True, size=(5, 1), key="isVGAout", disabled=False),
                      ...
                      [
                          sg.Push(),
                          sg.Frame(layout=[[sg.CBox('Widescreen', key='is_widescreen', disabled=True)]],
                                   title="Aspect Ratio", title_location=sg.TITLE_LOCATION_TOP, key="WIDE_MODE_BLOCK"),
                          sg.Push()
                      ],
                      [
                          sg.Frame(layout=[[sg.Radio('PAL (50Hz)', "SYS_COLOR_MODE", default=True, key="isPAL", disabled=False),
                                            sg.Radio('NTSC (60Hz)', "SYS_COLOR_MODE", key="isNTSC", disabled=False)]],
                                   title="Standard", title_location=sg.TITLE_LOCATION_TOP, key="C_MODE_BLOCK")
                      ]],
     ...
     sg.Frame(layout=[[sg.Radio('1X', "BUFMODE", default=True, size=(5, 1), key="is1X_BUFMODE", disabled=False),
                       sg.Radio('3X', "BUFMODE", key="is3X_BUFMODE", disabled=False)]],
              title="Buffering", title_location=sg.TITLE_LOCATION_TOP)
    ],

    [sg.Frame(layout=[[comboBoxPorts]], title="Port Selection",
              title_location=sg.TITLE_LOCATION_TOP, key='portNameBlock')]
]

block1 = [[
    syncSelect,
    sg.Frame(layout=[[comboBoxMode], [dividerF]],
             title="Pixel Clock Source", title_location=sg.TITLE_LOCATION_TOP)
],
...
block2 = [[sg.Frame(layout=[
    ...
    [sg.Push(), sg.Frame(layout=[[sliderDelay1, sliderDelay2],
                                 [sg.Push(), sliderDelay0, sg.Push()]],
                        title="Capture Delay", title_location=sg.TITLE_LOCATION_TOP, key='delay_cap_block'), sg.Push()],
...
# Draw the window
window = sg.Window('RGB to VGA & HDMI Settings', layout, font=('', 12), margins=(10, 0),
                   size=(600, 650), default_element_size=(40, 1), scaling=True, grab_anywhere=False)
...
while True:
    ...
    # Saving all settings
    if event == 'btn1Click':
        sg.PopupNoButtons("Rebooting board, please wait...", auto_close=True)
        l_mode = 'write'
    ...
    if modeName == namesMode[1]:  # External frequency
    ...
    if modeName == namesMode[2]:  # From Z80 pin 6
        window['dividerFblock'].hide_row()
        window['setting_F_Block'].hide_row()
    ...
    if event == '__TIMEOUT__':
        if ser is not None:
            try:
                ...
                # mode 0 - read/write parameters
                if hw_mode.find("mode 0") > 0:
                    print("hw mode 0")

                    # Save to chip
                    if l_mode == 'write':
                        sg.PopupNoButtons("Saving parameters...", auto_close=True)
                    ...
                    if l_mode == 'read':
                        ...
                        s1 = str(ser.readall())
                        i_data = int(''.join(x for x in s1 if x.isdigit()))
                        window['is_widescreen'].update(i_data == 1)
